...
final class MockAPIClient: APIClientProtocol {
    ...
    func createComment(postId: UUID, text: String) async throws -> Void {
        ...
        req.httpMethod = "POST"
        if let token = authToken { req.addValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }
        let body = ["operation": "create", "postid": postId.uuidString, "text": text]
        // send as form data
        let bodyString = body.map { "\($0.key)=\(String(describing: $0.value))" }.joined(separator: "&")
    ...
    func deleteComment(commentId: UUID) async throws -> Void {
        ...
        req.httpMethod = "POST"
        if let token = authToken { req.addValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }
        let body = ["operation": "delete", "commentid": commentId.uuidString]
        let bodyString = body.map { "\($0.key)=\(String(describing: $0.value))" }.joined(separator: "&")
        req.httpBody = bodyString.data(using: .utf8)
        req.addValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
        _ = try await session.data(for: req)
    ...
    func fetchComments(postId: UUID) async throws -> [Comment] {
        try await Task.sleep(nanoseconds: 80 * 1_000_000)
        return MockData.makeComments(for: postId)
    }
...
final class RESTAPIClient: APIClientProtocol {
    ...
    func createPost(authorId: UUID, imageData: Data?, caption: String?) async throws -> Post {
        ...
        if let data = imageData {
            ...
            var body: [String: Any] = [:]
            if let caption = caption { body["caption"] = caption }
            // Note: legacy server /posts expects multipart uploads; sending JSON without a file may be rejected.
            req.httpBody = try JSONSerialization.data(withJSONObject: body)
        }
